...
struct Cache {
    ...
    */
    ...
    bool is_hit(const paddr_t &addr) const {
        ...
        paddr_t tag = addr / (numSet * numBlock); // Extract tag from address
    ...
    bool access(
        ...
        paddr_t aligned_addr = addr & ~(numBlock - 1);

        paddr_t index = (aligned_addr / numBlock) % numSet; // Extract index from address
        paddr_t tag = aligned_addr / (numSet * numBlock); // Extract tag from address
    ...
    void update_cacheline(const paddr_t &addr, const uint8_t& data, const CPU_Cache_State &state) {
        paddr_t offset = addr % numBlock; // Extract offset from address
        paddr_t index = (addr / numBlock) % numSet; // Extract index from address
        paddr_t tag = addr / (numSet * numBlock); // Extract tag from address
    ...
    void update(const datflit_t &data) const {
